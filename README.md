...
This app is made for educational purposes (and a bit of fun) and is not intended to be used by people who don't know how to use it. **I am not responsible for any damages caused by this app so use it at your own risk**. Always make sure to backup your stuff before runing sussy apps like this one.
...
3. Make adjustments to the output using the selected options. You can either change the combo colour (one colour tho cause I reckon you ain't that insane to use RGB combo colours with a white insta-fade skin), backup some parts of your skin elements in case you want to rollback, process @2x elements or even create a triplestacked skin. Tooltip should give you enough information on what each option does. If not, just YOLO it :ujel:
...
![Windows Path](public/1.png)

Select all of it, then copy and paste that into the text box of the app like this, then press enter.

![Path Pasted](public/2.png)

If you do it correctly, it shouldn't show any error, and the first combo colour should be read and shown in the app

![Paste successful](public/3.png)

### osu!lazer (might be outdated in the future since they update their UI like once per week :ujel:)

osu!lazer is a bit more tricky since you have to export the skin to edit it. You can do that by going to `Skin layout editor > File > Edit externally`. Osu! then should mount the skin to a temp folder and open it in your file explorer. You can then follow the same steps as osu!stable and when you're done, go back to osu!lazer and select `Finish editing and importing changes` to import the skin back to the game.
